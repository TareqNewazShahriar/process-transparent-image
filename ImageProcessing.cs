...
public class ImageProcessing : IDisposable
{
    ...
    public ImageProcessing(byte[] imageBytes)
    {
    ...
    public void SetColorToTransparentPixels(Constants.FillTransparency fillTransparencyValue, bool smoothEdging)
    {
        int countTransparentPixels = 0;
        SKColor requestedColor = new SKColor();

        if (fillTransparencyValue == Constants.FillTransparency.White)
            requestedColor = new SKColor(byte.MaxValue, byte.MaxValue, byte.MaxValue, byte.MaxValue);
        else if (fillTransparencyValue == Constants.FillTransparency.Black)
    ...
    private SKColor SmoothPixel(SKColor color, Constants.FillTransparency fillTransparency)
    {
        SKColor newColor = color;
        int howLessIsAlpha = (byte.MaxValue - color.Alpha) / 2;

        if (fillTransparency == Constants.FillTransparency.White)
        {
            // make the color lighter
            newColor = new SKColor(
                (byte)(color.Red + howLessIsAlpha < byte.MaxValue ? color.Red + howLessIsAlpha : byte.MaxValue),
                (byte)(color.Green + howLessIsAlpha < byte.MaxValue ? color.Green + howLessIsAlpha : byte.MaxValue),
                (byte)(color.Blue + howLessIsAlpha < byte.MaxValue ? color.Blue + howLessIsAlpha : byte.MaxValue),
                byte.MaxValue);
        }
        else if (fillTransparency == Constants.FillTransparency.Black)
        {
            // make the color darker
            newColor = new SKColor(
                (byte)(color.Red - howLessIsAlpha > byte.MinValue ? color.Red - howLessIsAlpha : byte.MaxValue),
                (byte)(color.Green - howLessIsAlpha > byte.MinValue ? color.Green - howLessIsAlpha : byte.MaxValue),
                (byte)(color.Blue - howLessIsAlpha > byte.MinValue ? color.Blue - howLessIsAlpha : byte.MaxValue),
                byte.MaxValue);
        }
